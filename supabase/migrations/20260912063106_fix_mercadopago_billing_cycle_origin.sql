...
begin
  ...
  if found then
    ...
    return public.get_powerfit_payment_receipt(v_existing.id)
      || jsonb_build_object('version','mercadopago-plan-payment-v1','idempotent',true,'plan_code',v_plan);
  end if;
  ...
  return public.get_powerfit_payment_receipt(v_tx.id)
    || jsonb_build_object(
      'version','mercadopago-plan-payment-v1.1-calendar-membership-from-paid-date',
      'idempotent',false,
      'plan_code',v_plan,
